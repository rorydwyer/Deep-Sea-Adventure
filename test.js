...
export const ForbiddenDesert = {
        ...
        if (G.numMoves < 4) {
          ...
          if (G.tiles[currPos].type === "well") {
            ...
            for (var i = 0; i < G.tiles.length; i++) {
              if (G.tiles[i].isRevealed && G.tiles[i].type === "clue" && i !== currPos && G.tiles[i].part === G.tiles[currPos].part) {
                found = true;
              }
            }
            if (found) {
          ...
          if (val <= G.stormPicksUpProb || val <= G.sunBeatsDownProb) {
            ...
            if ((!bothQualify && val <= G.stormPicksUpProb) || (bothQualify && doStormPicksUp)) {
              ...
              for (var i = 0; i < G.players.length; i++) {
                ...
                if (!(G.tiles[G.players[i].position].type === "tunnel" && G.tiles[G.players[i].position].isRevealed) && !solarShielded) {
                  G.players[i].water -= 1;
                }
              }
            ...
            if (windDirection === 1) {
              ...
              while (affectedPos.length < maxTilesAffected) {
                ...
                if (candidatePos >= 0 && candidatePos <= 24 && Math.floor(candidatePos / 5) === Math.floor(stormPos / 5)) {
                  affectedPos.push(candidatePos);
                } else {
                  break;
                }
  ...
  endIf: (G, ctx) => {
    ...
    if (
      (ctx.numPlayers === 2 && G.stormLevel === 13) ||
